...
void insertarAlFinal(struct Lista & tad, int elemento,int sucursal, char letra) {

    struct Nodo * ultimoNodo = obtenerUltimoNodo(tad);
    struct Nodo * nuevoNodo = crearNodo(elemento,sucursal,letra, nullptr);
...
struct Nodo * crearNodo(int elemento,int sucursal,char letra, struct Nodo * siguiente) {
    struct Nodo * nuevoNodo = new struct Nodo;
    nuevoNodo->elemento = elemento;
...
void imprime(const struct Lista & tad) {
    ...
    if (esListaVacia(tad)) {
        ...
        while (recorrido != nullptr) {
            ...
            if(recorrido->letra == 'L' ) cout<<"(Lunes)";
            if(recorrido->letra == 'M' ) cout<<"(Martes)";
            if(recorrido->letra == 'm' ) cout<<"(Miercoles)";
            if(recorrido->letra == 'J' ) cout<<"(Jueves)";
            if(recorrido->letra == 'V' ) cout<<"(Viernes)";
            recorrido = recorrido->siguiente;
        }
        cout << "]" << endl;
    }
