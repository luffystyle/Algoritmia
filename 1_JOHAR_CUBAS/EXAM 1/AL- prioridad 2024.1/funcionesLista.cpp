...
void insertarencola(struct Lista & tad, int elemento, char*codigo) {

    struct Nodo * ultimoNodo = tad.cola;
    struct Nodo * nuevoNodo = crearNodo(elemento,codigo, nullptr);
    int mes=(elemento/100)%100;
    int anio=(elemento/100)/100;
    int dia=elemento%100;
    int edad = 2024-anio;
    ...
    if (ultimoNodo == nullptr){/*Si la lista está vacia*/
        tad.cabeza = nuevoNodo; /*se inserta en la cabeza de la lista*/
        tad.cola = nuevoNodo;
        if(edad>80) tad.may80 = nuevoNodo;
        if(edad<10) tad.men10 = nuevoNodo;
    } 
    else{ //La lista ya tiene nodos
        if(edad>80){
            if(tad.may80 ==nullptr){
                nuevoNodo->siguiente = tad.cabeza ;
                tad.cabeza = nuevoNodo;//falta la cola 
            }else{
                if(tad.may80->siguiente == nullptr){
                    tad.cola =nuevoNodo;
                }
                nuevoNodo->siguiente = tad.may80->siguiente;
                tad.may80->siguiente = nuevoNodo;
            }
            tad.may80 = nuevoNodo;
        }else if(edad<10){
            if(tad.men10==nullptr){
                if(tad.may80!=nullptr){
                    if(tad.may80->siguiente==nullptr){
                        tad.cola = nuevoNodo;
                    }
                ...
                nuevoNodo->siguiente = tad.men10->siguiente;
                tad.men10->siguiente = nuevoNodo;
                tad.men10 = nuevoNodo;
                
            }

        }else if(edad>=10 or edad<=80){
            ultimoNodo->siguiente = nuevoNodo;
            tad.cola = nuevoNodo;
        }
    }
...
struct Nodo * crearNodo(int elemento,char *codigo, struct Nodo * siguiente) {
    struct Nodo * nuevoNodo = new struct Nodo;
    nuevoNodo->elemento = elemento;
...
void imprime(const struct Lista & tad) {
    ...
    if (esListaVacia(tad)) {
        ...
        while (recorrido != nullptr) {
            ...
            cout << recorrido->elemento%100<<"/"<<(recorrido->elemento/100)%100<<"/"<<(recorrido->elemento/100)/100;
