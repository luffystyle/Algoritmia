...
void insertarencola(struct Lista & tad,char prioridad , int elemento) {
    ...
    if (ultimoNodo == nullptr){/*Si la lista está vacia*/
        tad.cabeza = nuevoNodo; /*se inserta en la cabeza de la lista*/
        tad.cola = nuevoNodo;
        if( prioridad == 'V') tad.vip = nuevoNodo;
        if( prioridad == 'C') tad.cliente = nuevoNodo;
    } 
    else{ //La lista ya tiene nodos
        
        if(prioridad == 'V' || prioridad == 'C'){
            
            if(prioridad == 'V'){
                
                if(tad.vip == nullptr and tad.cliente == nullptr){
                    //SI ES QUE NO HAY NODOS DE VIP Y CLIENTE SOLO HAY NO CLIENTE
                    //nuevoNodo->siguiente=tad.cabeza;
                    tad.vip = nuevoNodo;
                    tad.vip->siguiente = tad.cabeza;
                    tad.cabeza=nuevoNodo;
                }else if(tad.vip != nullptr and tad.cliente == nullptr){
                    //si es que ya hay un nodo de VIP
                    ...
                    //CUANDO HAY NODOS DE CLIENTES Y NO CLIENTES
                    tad.vip = nuevoNodo;
                    tad.vip->siguiente = tad.cabeza;
                    tad.cabeza = nuevoNodo;
                }else if(tad.vip != nullptr and tad.cliente != nullptr){
                    //cuando hay nodos de vip y nodos de cliente y nodos de no clientes
                    nuevoNodo->siguiente = tad.vip->siguiente;
                    tad.vip->siguiente= nuevoNodo;
                    tad.vip=nuevoNodo;
            ...
            if(prioridad == 'C'){
                if(tad.cliente == nullptr and tad.vip == nullptr){
                    //cuando solo hay nodos de no cliente
                    tad.cliente = nuevoNodo;
                    tad.cliente->siguiente=tad.cabeza;
                    tad.cabeza =nuevoNodo;
                    
                }else if(tad.cliente != nullptr and tad.vip == nullptr){
                    if(tad.cliente->siguiente==nullptr){
                        tad.cola = nuevoNodo;
                    }
                    nuevoNodo->siguiente=tad.cliente->siguiente;
                    tad.cliente->siguiente = nuevoNodo;
                    tad.cliente = nuevoNodo;
                    
                    
                }else if(tad.cliente == nullptr and tad.vip != nullptr){
                    //si hay nodos de vip y no clientes
                    if(tad.vip->siguiente==nullptr){
                        tad.cola=nuevoNodo;
                    }
                    tad.cliente = nuevoNodo;
                    tad.cliente->siguiente = tad.vip->siguiente ;
                    tad.vip->siguiente= nuevoNodo;
...
//        if(prioridad!='V' and prioridad!='C'){
//            tad.cola->siguiente= ultimoNodo;
//            tad.cola= ultimoNodo;
//        }       
    }
    tad.longitud++;
