...
void plantarArbolBB(struct NodoArbol *& arbol, 
                    struct NodoArbol * arbolIzquierdo, int elemento, int nivel,
...
void insertarRecursivo(struct NodoArbol *& raiz, int elemento, int nivel){
    if(esNodoVacio(raiz))
        plantarArbolBB(raiz, nullptr, elemento,nivel, nullptr);
    else
        if (raiz->elemento > elemento)
            insertarRecursivo(raiz->izquierda, elemento,nivel);
        else
            if(raiz->elemento < elemento)
                insertarRecursivo(raiz->derecha, elemento,nivel);
            else
                cout<<endl<<"El elemento "<< elemento <<" ya se encuentra en el árbol"<< endl;
}

void insertar(struct ArbolBinarioBusqueda & arbol, int elemento,int nivel){
    insertarRecursivo(arbol.arbolBinario.raiz, elemento,nivel);
}
