...
void plantarArbolBB(struct NodoArbol *& arbol, 
                    struct NodoArbol * arbolIzquierdo, struct Elemento elemento, 
...
void insertarRecursivo(struct NodoArbol *& raiz, struct Elemento elemento){
    if(esNodoVacio(raiz))
        plantarArbolBB(raiz, nullptr, elemento, nullptr);
    else{
        if (raiz->elemento.campo1 > elemento.campo1 ){
            insertarRecursivo(raiz->izquierda, elemento);
        }else{
            if(raiz->elemento.campo1 < elemento.campo1){
                insertarRecursivo(raiz->derecha, elemento);
            }else{
                if(raiz->elemento.campo2 <  elemento.campo2){
                    insertarRecursivo(raiz->derecha,elemento);
                }else{
                    if(raiz->elemento.campo2 >  elemento.campo2){
                        insertarRecursivo(raiz->izquierda,elemento);
                    }else{
                        cout<<endl<<"El elemento "<< elemento.campo1 <<" ya se encuentra en el árbol"<< endl;
                    }
                }
            }
        }
...
void insertar(struct ArbolBinarioBusqueda & arbol, struct Elemento elemento){
...
struct NodoArbol * eliminarNodoRecursivo(struct NodoArbol * nodo, int elemento){
    ...
    if(nodo->elemento.campo1 > elemento)
        ...
        if(nodo->elemento.campo1 < elemento)
            ...
            if(esNodoVacio(nodo->izquierda)){
                ...
                if(esNodoVacio(nodo->derecha)){
                    ...
                    nodo->derecha = eliminarNodoRecursivo(nodo->derecha, minNodo->elemento.campo1);
                }   
    return nodo;
...
bool buscaArbolRecursivo(struct NodoArbol * nodo, int dato){
    ...
    if(comparaABB(nodo->elemento.campo1, dato) == 0)
        return true;
    if(comparaABB(nodo->elemento.campo1, dato) == 1)
