...
void insertarRecursivo(struct NodoArbol *& raiz, struct Lote elemento){
    if(esNodoVacio(raiz))
        ...
        if (raiz->elemento.codigo > elemento.codigo)
            insertarRecursivo(raiz->izquierda, elemento);
        else
            if(raiz->elemento.codigo < elemento.codigo)
                insertarRecursivo(raiz->derecha, elemento);
            else
                cout<<endl<<"El elemento "<< elemento.codigo <<" ya se encuentra en el árbol"<< endl;
}
...
struct NodoArbol * eliminarNodoRecursivo(struct NodoArbol * nodo, int elemento){
    ...
    if(nodo->elemento.codigo > elemento)
        ...
        if(nodo->elemento.codigo < elemento)
            ...
            if(esNodoVacio(nodo->izquierda)){
                ...
                if(esNodoVacio(nodo->derecha)){
                    ...
                    nodo->elemento.codigo = minNodo->elemento.codigo;
                    nodo->elemento.cantidad = minNodo->elemento.cantidad;
                    /*se elimina el nodo que se reemplazo*/
                    nodo->derecha = eliminarNodoRecursivo(nodo->derecha, minNodo->elemento.codigo);
                }   
    return nodo;
...
bool buscaArbolRecursivo(struct NodoArbol * nodo, int dato){
    ...
    if(comparaABB(nodo->elemento.codigo, dato) == 0)
        return true;
    if(comparaABB(nodo->elemento.codigo, dato) == 1)
