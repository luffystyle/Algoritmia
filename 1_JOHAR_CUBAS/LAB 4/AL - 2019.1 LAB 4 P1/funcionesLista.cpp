...
//struct Nodo * seEncuentra(const struct Lista & tad, struct NodoArbol* elemento) {
//    struct Nodo * recorrido = tad.cabeza;
//
...
void eliminaNodo(struct Lista & tad, struct NodoArbol* elemento) {
    struct Nodo * ultimo = nullptr;
    struct Nodo * recorrido = tad.cabeza;
...
void imprime(const struct Lista & tad) {
    ...
    if (esListaVacia(tad)) {
        ...
        while (recorrido != nullptr) {
            ...
            cout << recorrido->elemento->elemento.codigo<<"-"<<recorrido->elemento->elemento.cantidad<<" " ;
            recorrido = recorrido->siguiente;
        }
        cout << "]" << endl;
    }
