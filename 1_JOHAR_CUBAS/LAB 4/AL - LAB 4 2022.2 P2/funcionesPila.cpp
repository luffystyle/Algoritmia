...
void apilar(struct Pila & pila, struct Lote &elemento){
...
struct Lote desapilar(struct Pila & pila){
    
    if (esPilaVacia(pila)){
        cout<<"La pila está vacía, por lo tanto no se puede desapilar"<<endl;
	Lote lote1;
        lote1.codigo = -1;
        return lote1; //O cualquier otro valor que no se encuentre en la pila
    }	
    else{
        Lote elementoAux = cima(pila);
...
struct Lote cima(const struct Pila & pila){
	...
	cout<<"La pila está vacía por lo tanto no posee cima"<<endl;
        ...
        lote.cantidad = -1;
        lote.cantidad = -1;
        return lote; //O cualquier otro valor que no se encuentre en la pila
    }		
