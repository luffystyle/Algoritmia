...
void insertarAlFinal(struct Lista & tad, struct NodoArbol *elemento) {
...
void imprime(const struct Lista & tad) {
    ...
    if (esListaVacia(tad)) {
        ...
        while (recorrido != nullptr) {
            ...
            cout << recorrido->arbol->elemento<<"-"<<recorrido->arbol->cantidad<<" ";
            recorrido = recorrido->siguiente;
        }
        cout << "]" << endl;
    }
