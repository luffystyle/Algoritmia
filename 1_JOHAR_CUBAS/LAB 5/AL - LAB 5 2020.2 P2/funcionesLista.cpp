...
void insertarAlInicio(struct Lista & tad, int elemento,int cant) {
...
struct Nodo * crearNodo(int elemento,int cant ,struct Nodo * siguiente) {
    struct Nodo * nuevoNodo = new struct Nodo;
    nuevoNodo->elemento = elemento;
    nuevoNodo->cantidad = cant;
...
void imprime(const struct Lista & tad) {
    ...
    if (esListaVacia(tad)) {
        ...
        while (recorrido != nullptr) {
            ...
            cout << recorrido->elemento<<"-"<<recorrido->cantidad<<" ";
            recorrido = recorrido->siguiente;
        }
        cout << "]" << endl;
    }
