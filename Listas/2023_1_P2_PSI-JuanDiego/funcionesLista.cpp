...
void insertarAlInicio(struct Lista & tad, int nivel_impacto, char tipo_categoria) {
    ...
    nuevoNodo->tipo_categoria = tipo_categoria;
...
void insertarAlFinal(struct Lista & tad,  int nivel_impacto, char tipo_categoria) {

    struct Nodo * ultimoNodo = obtenerUltimoNodo(tad);
    struct Nodo * nuevoNodo = crearNodo(nivel_impacto, tipo_categoria, nullptr);
...
void imprime(const struct Lista & tad) {
    ...
    if (esListaVacia(tad)) {
        ...
        while (recorrido != nullptr) {
            ...
            cout << "( "<<recorrido->tipo_categoria << "-"<<recorrido->nivel_impacto<<") ";
            recorrido = recorrido->siguiente;
        }
        cout << "]" << endl;
    }
