...
void solucion(int *cromosoma, int *camiones, int *paquetes, int combinaciones, int n, int cantEstados, int indice, int m, int &minimaDiff,int *solucionPaquetes){
    
    int auxCamion[m], mayor, menor ; // este auxiliar vamos ir cambiando porque las capacidades se actualizan
    for(int i = 0; i<m; i++) auxCamion[i] = camiones[i];
    int postCamion;
...
int main(int argc, char** argv) {
    ...
    for( int i = 0; i<combinaciones; i++) {
        ...
        solucion(cromosoma, camiones, paquetes, combinaciones, n, cantEstados, i, m, minimaDiff, solucionPaquetes);
    }
