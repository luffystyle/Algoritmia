...
void encolar(struct Cola & cola, int codigo,int prioridad){
    insertarencola(cola.lista, codigo,prioridad);
    //cout<<"Encolando: "<<elemento<<endl;
}

/*Elimina un elemento de la cola*/
struct Nodo * desencolar(struct Cola & cola){
    if(esColaVacia(cola)){
        cout<<"La cola está vacía no se puede desencolar"<<endl;
        //return -1; //Cero o algún elemento que no esté en la cola
    }
    else{
        Nodo * elemento = retornaCabeza(cola.lista);
        eliminaCabeza(cola.lista);
        if(esColaVacia(cola))
            cola.lista.cola = nullptr;
        return cola.lista.cabeza;
    }
