...
struct NodoArbol{
    ...
    int cantidad;  //Parte a. Este dato es el valor que nos pide el enunciado
...
struct NodoArbol * crearNuevoNodoArbolNew(struct NodoArbol * arbolIzquierdo, 
                               int elemento, int cantidad, struct NodoArbol * arbolDerecho){
    struct NodoArbol * nuevoNodo = new struct NodoArbol;
    nuevoNodo->elemento = elemento;
...
void plantarArbolBinarioNew(NodoArbol * &raiz, struct NodoArbol * arbolIzquierdo,
                         int elemento, int cantidad, struct NodoArbol * arbolDerecho){
    
    struct NodoArbol * nuevoNodo = crearNuevoNodoArbolNew(arbolIzquierdo, elemento, cantidad, arbolDerecho);
    raiz = nuevoNodo;
}
...
void insertarRecursivoNew(struct NodoArbol * &raiz, int elemento, int cantidad){
    if(raiz == NULL)
        plantarArbolBinarioNew(raiz, NULL, elemento, cantidad, NULL);
    else{
    	insertarRecursivoNew(raiz->izquierda,0,cantidad);
    	insertarRecursivoNew(raiz->derecha,1,cantidad);
	}
}

/*Parte a*/
void insertarNew(struct ArbolBinario & arbol, int elemento, int cantidad){
    insertarRecursivoNew(arbol.raiz, elemento, cantidad);
}
...
/*void determinaCombinaciones(NodoArbol *raiz,int peso, int suma){
	if (raiz!=NULL){
		suma = suma + raiz->elemento*raiz->cantidad;
		determinaCombinaciones(raiz->izquierda,peso,suma);
		determinaCombinaciones(raiz->derecha,peso,suma);
	}
	else {
		if (suma==peso){
			cantidad++;
...
void determinaCombinaciones(NodoArbol *raiz,int peso, int suma){
	suma = suma + raiz->cantidad*raiz->elemento;
...
int main(){
	...
	int elementos[N] = {10,50,20,30,40};
	int n=5;
	/*Parte a*/
	construir(arbol);
	insertarNew(arbol,0,0);
	for (int i=0; i<n; i++){
		insertarNew(arbol,0,elementos[i]); /*El valor de 0 es uno que no se utiliza*/
	}
	recorrerPreOrdenNew(arbol);
	/*Parte b*/
	int peso = 70;
	int suma = 0;
	determinaCombinaciones(arbol.raiz,peso,suma);
	cout << "Hay " << cantidad << " soluciones validas";
	return 0;
}
