...
void recorridoPorNivel(struct ArbolBinario arbol){
    ...
    construir(cola);//iniciamos la Cola
    if(not esArbolVacio(arbol)){//Verificamos que el arbol tenga nodos
        encolar(cola, arbol.raiz);//antes de entrar a la iteraci�n, encolamos la raiz del �rbol
        while(not esColaVacia(cola)){//La Cola es la que va a ir "guardando" los hijos de los nodos que vamos visitando
            struct NodoArbol * nodo = desencolar(cola);//Obtenemos el nodo de la Cola, para imprimirlo y visitar a sus hijos
            imprimeNodo(nodo);
            if (not esNodoVacio(nodo->izquierda)){//si existe el hijo izquierdo, lo encolo
                encolar(cola, nodo->izquierda);
            }
            if (not esNodoVacio(nodo->derecha)){//si existe el hijo derecho, lo encolo
...
/*recorre el árbol por niveles usando una cola*/
void recorridoPorNivelInverso(struct ArbolBinario arbol){
    struct Cola cola; /*Se usa una cola para acceder a los nodos*/
    construir(cola);//iniciamos la Cola
    struct Pila pila;/*Se usa una pila para "guardar" los nodos visitados y trabajados*/
    construir(pila);//iniciamos la Pila
    ...
    if(not esArbolVacio(arbol)){//Verificamos que el arbol tenga nodos
        ...
        while(not esColaVacia(cola)){//La Cola es la que va a ir "guardando" los hijos de los nodos que vamos visitando
            struct NodoArbol * nodo = desencolar(cola);//Obtenemos el nodo de la Cola, para imprimirlo y visitar a sus hijos
            apilar(pila, nodo);
            //imprimeNodo(nodo); Para recorrido inverso no imprimimos en este punto.
            if (not esNodoVacio(nodo->izquierda)){//si existe el hijo izquierdo, lo encolo
                encolar(cola, nodo->izquierda);
            }
            if (not esNodoVacio(nodo->derecha)){//si existe el hijo derecho, lo encolo
...
void recorridoPorNivelInverso(struct ArbolBinarioBusqueda arbol){
    recorridoPorNivelInverso(arbol.arbolBinario);
}
...
int main(int argc, char** argv) {
	...
	cout<<"Recorrido por Nivel Inverso:"<<endl;
    recorridoPorNivelInverso(arbol);
