...
int main(){
	...
	int ganancias[TOTAL] = {10,10,10,5,15,10,10,10,10,20,10,5,10,10,5,10};
	...
	for (int i=0; i<combinaciones; i++){
		...
		for (int j=0; j<n*4; j++){
			if (cromo[j]==1){
				...
				gananciaParcial = gananciaParcial + ganancias[j];
			}
		}
		if (peso>=pesoParcial && gananciaMaxima<gananciaParcial){
			soluciones[0] = i;
