...
void fusionarRecursivo(struct NodoArbol * nodo1, struct NodoArbol * nodo2){
	...
	if (nodoAux){
		...
		//Antes de insertar el valor del nodo, se tiene que verificar que:
		//El nodo no exista en el �rbol de destino.
		if (!buscaArbolRecursivo(nodo1,valor))
			insertarRecursivo(nodo1,valor);//insertamos con confianza en el �rbol destino.
		nodo2=NULL;//Al puntero le asignamos NULL como �ltima acci�n de su eliminaci�n.
	}
}

void fusionar(struct ArbolBinarioBusqueda * arbol1, struct ArbolBinarioBusqueda * arbol2){
...
int main(int argc, char** argv) {
    ...
    fusionar(&arbol1, &arbol2);
    cout<<"FUSION - fin"<<endl;
	cout<<"Arbol 1: ";
    preOrden(arbol1); 
    cout<<endl;
