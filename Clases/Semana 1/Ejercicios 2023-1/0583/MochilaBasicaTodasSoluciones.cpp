...
//Quiero hallar todas las mejores soluciones

//Primero recorro todas las combinaciones para hallar la mejor solucion
//Luego recorro nuevamente todas las combinaciones para mostrar todas en donde coincida con la mejor solucion
...
int main(){
	...
	for (int z=1; z<combinaciones; z++){
		cargaBinaria(z,cromosoma,N); //convertimos la combinacion a binario
		peso_mochila = 0; //inicializo en 0 el peso de la combinacion
		for (int j=0; j<N; j++){//Voy a calcular el peso de la combinacion
			if (cromosoma[j]==1)//cada vez que encuentre un objeto/caja asignado, acumulo el peso
				peso_mochila = peso_mochila + objetos[j];//acumulo el peso
		}
		if (peso_mochila==mejor_peso_mochila){
			cout << "Combinacion " << z << ": "; //Muestro la solucion
			for (int j=0; j<N; j++){
					cout << cromosoma[j] << "  "; //Se muestra en binario
			}
			cout << endl;
			for (int j=0; j<N; j++){
				if (cromosoma[j]==1){
