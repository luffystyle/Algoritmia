...
void insertarRecursivo(struct NodoArbol *& raiz, struct Elemento elemento){
    if(esNodoVacio(raiz))
        ...
        if(raiz->elemento.posicion > elemento.posicion)
            insertarRecursivo(raiz->izquierda, elemento);
        else
            if(raiz->elemento.posicion < elemento.posicion)
                insertarRecursivo(raiz->derecha, elemento);
            else
                cout << "El elemento " << elemento.posicion << "Ya se encuentra en el árbol" << endl;
}
...
bool buscarRecursivo(struct NodoArbol * nodo, int dato){
    if(esNodoVacio(nodo))
        return false;
    if(comparaABB(nodo->elemento.posicion, dato) == 0)
        return true;
    if(comparaABB(nodo->elemento.posicion, dato) == 1)
...
struct NodoArbol * eliminarRecursivo(struct NodoArbol * nodo, int elemento){
    ...
    if(nodo->elemento.posicion > elemento)
        ...
        if(nodo->elemento.posicion < elemento)
            ...
            if(esNodoVacio(nodo->izquierda)){
                ...
                if(esNodoVacio(nodo->derecha)){
                    ...
                    nodo->derecha = eliminarRecursivo(nodo->derecha, minNodo->elemento.posicion);
                }
