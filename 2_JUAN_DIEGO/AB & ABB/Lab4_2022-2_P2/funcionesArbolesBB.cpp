...
void plantarArbolBB(struct NodoArbol *& arbol, 
                    struct NodoArbol * arbolIzquierdo, int elemento,int dato,
    ...
    void insertarRecursivo(struct NodoArbol *& raiz, int elemento,int dato){
    if(esNodoVacio(raiz))
        plantarArbolBB(raiz, nullptr, elemento,dato, nullptr);
    else
        if (raiz->elemento > elemento)
            insertarRecursivo(raiz->izquierda, elemento,dato);
        else
            if(raiz->elemento < elemento)
                insertarRecursivo(raiz->derecha, elemento,dato);
            else
                cout<<endl<<"El elemento "<< elemento <<" ya se encuentra en el árbol"<< endl;
}

void insertar(struct ArbolBinarioBusqueda & arbol, int elemento,int dato){
    insertarRecursivo(arbol.arbolBinario.raiz, elemento,dato);
}
