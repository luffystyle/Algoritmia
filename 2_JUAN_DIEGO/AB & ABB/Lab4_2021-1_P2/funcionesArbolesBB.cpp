        ...
        if(nodo->total < elemento)
            ...
            if(esNodoVacio(nodo->izquierda)){
                ...
                if(esNodoVacio(nodo->derecha)){
                    ...
                    nodo->total = minNodo->total;
                    /*se elimina el nodo que se reemplazo*/
                    nodo->derecha = eliminarNodoRecursivo(nodo->derecha, minNodo->total);
                }   
    return nodo;
...
bool buscaArbolRecursivo(struct NodoArbol * nodo, int dato){
    ...
    if(comparaABB(nodo->total, dato) == 0)
        return true;
    if(comparaABB(nodo->total, dato) == 1)
