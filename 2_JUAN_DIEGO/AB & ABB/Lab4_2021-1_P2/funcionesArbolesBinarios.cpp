...
void imprimeNodo(struct NodoArbol * nodo){
    cout<<setw(5)<<nodo->fecha<<"||"<<nodo->dni<<"||"<<nodo->numAcciones<<"||"<<nodo->total<<"||"<<nodo->estado<<endl;
}
...
int sumarNodosRecursivo(struct NodoArbol * nodo){
    if(esNodoVacio(nodo))
        ...
        return nodo->total + sumarNodosRecursivo(nodo->izquierda) + sumarNodosRecursivo(nodo->derecha);
}
