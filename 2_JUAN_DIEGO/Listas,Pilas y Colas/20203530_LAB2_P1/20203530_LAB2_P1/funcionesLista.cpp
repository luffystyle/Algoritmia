...
void construir(struct Lista & tad) {
    tad.cabeza = nullptr;
    tad.cola = nullptr;
    tad.mayor= nullptr; //Apuntará a los mayores de 80 años
    tad.menor = nullptr; //Apuntará a los valores de los menores a 10 años
    tad.longitud = 0;
}
...
void insertarencola(struct Lista & tad, int dia, int mes, int anio, char* codigo) {

    struct Nodo * ultimoNodo = tad.cola;
    struct Nodo * nuevoNodo = crearNodo(dia, mes, anio, codigo, nullptr);
...
struct Nodo * crearNodo(int dia, int mes, int anio, char* codigo, struct Nodo * siguiente) {
...
void imprime(const struct Lista & tad) {
    ...
    if (esListaVacia(tad)) {
        ...
        while (recorrido != nullptr) {
            ...
            cout<<setfill('0')<<setw(2)<< recorrido->dia<<"/"
                    <<setw(2)<<recorrido->mes<<setfill(' ')<<"/"<< recorrido->anio
                    <<" "<<recorrido->codigo;
            recorrido = recorrido->siguiente;
        }
        cout << "]" << endl;
