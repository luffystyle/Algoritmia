...
void insertarAlInicio(struct Lista & tad, int deteccion,int turno) {

    /*Se va cambiar por la función crear nodo*/
    struct Nodo * nuevoNodo = new struct Nodo;
    nuevoNodo->deteccion = deteccion;
...
void insertarencola(struct Lista & tad, int codigo,int prioridad) {

    struct Nodo * ultimoNodo = tad.cola;
    struct Nodo * nuevoNodo = crearNodo(codigo,prioridad, nullptr);
...
struct Nodo * crearNodo(int deteccion,int turno, struct Nodo * siguiente) {
    struct Nodo * nuevoNodo = new struct Nodo;
    nuevoNodo->deteccion = deteccion;
    nuevoNodo->turno = turno;
...
struct Nodo * retornaCabeza(const struct Lista & tad){
    return tad.cabeza;
}
...
//            estaImprimiendoLaCabeza = 0;
            cout <<"PSI00"<< recorrido->turno<<","<<recorrido->deteccion;
            recorrido = recorrido->siguiente;
            cout << "]" ;
        }
