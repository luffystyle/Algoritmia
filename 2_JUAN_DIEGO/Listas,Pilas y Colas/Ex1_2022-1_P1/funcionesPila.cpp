...
void apilar(struct Pila & pila, int fecha,int id){
    insertarAlInicio(pila.lista, fecha,id);
    //cout<<"Apilando: "<<elemento<<endl;
}

/* pop, elimina un elemento de la parte superior de la pila*/
Nodo * desapilar(struct Pila & pila){
	...
	//return -1; //O cualquier otro valor que no se encuentre en la pila
        ...
        int fecha = pila.lista.cabeza->fecha;
        int ID = pila.lista.cabeza->ID;
        eliminaCabeza(pila.lista);
        return pila.lista.cabeza;
    }
}

/*examina un elemento situado en la parte superior de la pila*/
Nodo * cima(const struct Pila & pila){
