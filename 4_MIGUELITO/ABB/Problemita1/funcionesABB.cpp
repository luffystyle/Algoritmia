...
void insertarRecursivo(struct NodoArbol *& raiz, Elemento elemento){
    if(esNodoVacio(raiz))
        ...
        if(raiz->elemento.numeroLote > elemento.numeroLote)
            insertarRecursivo(raiz->izquierda, elemento);
        else
            if(raiz->elemento.numeroLote < elemento.numeroLote)
                insertarRecursivo(raiz->derecha, elemento);
            else
                cout << "El elemento " << elemento.numeroLote << '-' << elemento.cantidad << "Ya se encuentra en el árbol" << endl;
}
...
bool buscarRecursivo(struct NodoArbol * nodo, int dato){
    if(esNodoVacio(nodo))
        return false;
    if(comparaABB(nodo->elemento.numeroLote, dato) == 0)
        return true;
    if(comparaABB(nodo->elemento.numeroLote, dato) == 1)
...
struct NodoArbol * eliminarRecursivo(struct NodoArbol * nodo, int elemento){
    ...
    if(nodo->elemento.numeroLote > elemento)
        ...
        if(nodo->elemento.numeroLote < elemento)
            ...
            if(esNodoVacio(nodo->izquierda)){
                ...
                if(esNodoVacio(nodo->derecha)){
                    ...
                    nodo->derecha = eliminarRecursivo(nodo->derecha, minNodo->elemento.numeroLote);
                }
