...
void insertarAlInicio(struct Lista & lista, int elemento1,int elemento2){

    /*Se va cambiar por la función crear nodo*/
    struct Nodo * nuevoNodo = new struct Nodo;
    nuevoNodo->elemento.hora=elemento1;
    nuevoNodo->elemento.sucursal=elemento2;
...
void insertarAlFinal(struct Lista & lista, int elemento1,int elemento2) {

    struct Nodo * ultimoNodo = obtenerUltimoNodo(lista);
    struct Nodo * nuevoNodo = crearNodo(elemento1,elemento2, nullptr);
...
struct Nodo * crearNodo(int elemento1,int elemento2, struct Nodo * siguiente) {
    struct Nodo * nuevoNodo = new struct Nodo;
    nuevoNodo->elemento.hora = elemento1;
    nuevoNodo->elemento.sucursal = elemento2;
...
void insertarEnOrden(struct Lista & lista, int elemento1,int elemento2) {

    struct Nodo * nodoAnterior = obtenerNodoAnterior(lista, elemento1);
    struct Nodo * nuevoNodo = crearNodo(elemento1,elemento2, nullptr);
...
struct Nodo * obtenerNodoAnterior(const struct Lista & lista, int elemento1) {
    struct Nodo * anterior = nullptr;
    struct Nodo * recorrido = lista.cabeza;
    while ((recorrido != nullptr) and (recorrido->elemento.hora < elemento1)) {
        anterior = recorrido;
        recorrido = recorrido->siguiente;
    }
    return anterior;
...
struct Nodo * seEncuentra(const struct Lista & lista, int elemento1,int elemento2) {
    struct Nodo * recorrido = lista.cabeza;

    /*retorna el nodo correspondiente al elemento buscado*/
    while ((recorrido != nullptr) and (recorrido->elemento.hora != elemento1 && 
                                       recorrido->elemento.sucursal != elemento2)) {
        recorrido = recorrido->siguiente;
    }
    return recorrido;
}

void eliminaNodo(struct Lista & lista, int elemento1,int elemento2) {
    struct Nodo * ultimo = nullptr;
    struct Nodo * recorrido = lista.cabeza;
...
//        cout << "La lista está vacía no se puede mostrar" << endl;
        cout << "Este día no tuvo pedidos" << endl;
        return;
    } else {
        struct Nodo * recorrido = lista.cabeza;
