...
void imprime(const struct Lista & tad) {
    ...
    if (esListaVacia(tad)) {
        ...
        while (recorrido != nullptr) {
            ...
            int aa = recorrido->elemento.fecha/10000;
            int mm = (recorrido->elemento.fecha%10000)/100;
            int dd = recorrido->elemento.fecha%100;
            cout << dd << '/' << mm << '/' << aa << ' ' << recorrido->elemento.codigo;
            recorrido = recorrido->siguiente;
        }
        cout << "]" << endl;
    }
