...
void insertarencola(struct Lista & tad, long long elemento) {
...
struct Nodo * crearNodo(long long elemento, struct Nodo * siguiente) {
...
long long retornaCabeza(const struct Lista & tad){
...
void imprime(const struct Lista & tad) {
    ...
    if (esListaVacia(tad)) {
        ...
        cout << "AVION" << setw(18) << "Hora de llegada" << setw(25) << "Nuevo hora de llegada" << endl;
...
//                cout << ", ";
            ...
            int numVuel = recorrido->elemento/100000000;
            int horaIni = ((recorrido->elemento%10000000)/10000)/100;
            int minIni = ((recorrido->elemento%10000000)/10000)%100;
            int horaFin = (recorrido->elemento%10000)/100;
            int minFin = recorrido->elemento%100;
            cout << right;
            cout << numVuel << setw(10) << horaIni << ':' << minIni << setw(18)<< horaFin << ':' << minFin << endl;
            recorrido = recorrido->siguiente;
        }
