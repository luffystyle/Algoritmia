...
void recorrerEnOrden(const struct ArbolBinario & arbol, int &i){
    recorrerEnOrdenRecursivo(arbol.raiz, i);
}
void recorrerEnOrdenRecursivo(struct NodoArbol * nodo, int &i){
    if (not esNodoVacio(nodo)){
        i++;
        recorrerEnOrdenRecursivo(nodo->izquierda, i);
        imprimirNodo(nodo);
        
        recorrerEnOrdenRecursivo(nodo->derecha, i);
    }
}
