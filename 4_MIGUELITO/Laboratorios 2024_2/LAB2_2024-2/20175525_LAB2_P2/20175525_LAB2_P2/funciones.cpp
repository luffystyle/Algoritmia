...
void insertarAlFinal(struct Lista & lista, Pedido ped) {

    struct Nodo * ultimoNodo = obtenerUltimoNodo(lista);
    struct Nodo * nuevoNodo = crearNodo(ped, nullptr);
...
struct Nodo * crearNodo(Pedido ped, struct Nodo * siguiente) {
    struct Nodo * nuevoNodo = new struct Nodo;
    nuevoNodo->pedido = ped;
...
void imprime(const struct Lista & lista) {
    ...
    if (esListaVacia(lista)) {
        ...
        while (recorrido != nullptr) {
            ...
            cout << recorrido->pedido.cadena << "  " ;
            cout << recorrido->pedido.tPreparacion + recorrido->pedido.tViaje ;
            recorrido = recorrido->siguiente;
        }
        cout << "]"<< "  ";
    }
