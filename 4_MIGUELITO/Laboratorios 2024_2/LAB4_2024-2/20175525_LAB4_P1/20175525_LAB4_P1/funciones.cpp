...
struct Nodo * obtenerNodoAnterior(const struct Lista & lista, char elemento) {
...
struct Nodo * seEncuentra(const struct Lista & lista, char elemento) {
    struct Nodo * recorrido = lista.cabeza;
...
void eliminaNodo(struct Lista & lista, char elemento) {
    struct Nodo * ultimo = nullptr;
    struct Nodo * recorrido = lista.cabeza;
